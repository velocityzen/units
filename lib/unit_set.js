...
UnitSet.prototype.addSet = function(key, unitSet, parentKey) {
  if (key && this.unitInfoDict[key]) {
    ...
    let keyUnitSet = this.get(key);
    keyUnitSet.addSet(null, unitSet, key);
    return;
  }

  unitSet.setParent(this);
  let parent = parentKey && this.parent;
  let setUnitInfoDict = unitSet.unitInfoDict;
...
UnitSet.prototype.getLocal = function(key, local, init) {
  let unitInfo = this.unitInfoDict[key];
  if (unitInfo) {
...
UnitSet.prototype.getFromLoader = function(key, local, init) {
  let result;
  if (this.loader) {
    let unitToAdd = this.loader.loadUnit(key, local, init);
    if (unitToAdd) {
      let unitInfo = this.addInternal(key, new UnitInfo(unitToAdd, this));
...
UnitSet.prototype.require = function(key, local, init) {
  let unit = this.get(key, local, init);
  if (!unit) {
